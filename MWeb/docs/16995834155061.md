...
在 cgroup v1 中，cgroup writeback 迟迟无法实现，block cgroup 只能对 read 以及direct write 产生的 IO 进行统计并限流，而不能统计 buffer write 产生的 IO。这是因为 writeback 过程中，dirty page cache 的回写是由 worker thread 发起的，此时一开始往这个 page cache 写入数据的进程信息已经丢失，因而就不能确定将这个 page cache 回写产生的 IO 统计到哪个 block cgroup 下面。因而在此之前所有 writeback 产生的 IO 都是统计在 root block cgroup 下面
