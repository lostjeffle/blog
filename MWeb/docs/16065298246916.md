...
workqueue 是 Linux 中非常重要的一种异步执行的机制，在中断语境下 workqueue (还有 softirq/tasklet) 又称为“底半部” (bottom half)

workqueue 以 v2.6.36 为界，存在两个不同的实现，在此之前的实现我们称为 old-style，之后的即所谓的 CMWQ (Concurrency Managed Workqueue)
...
在介绍 old-style workqueue 的问题之前，其实有必要简单介绍一下 old-style workqueue 中 singlethread workqueue 和 multi-thread workqueue 的概念，其中前者整个 workqueue 只有一个 worker thread，而后者每个 workqueue 有一组 worker thread

> issue 1

首先 old-style workqueue 会创建出大量的 worker thread，即使其中的大部分可能都无所事事。对于 multi-thread workqueue 来说，每个 workqueue 都分配有一组 per-CPU worker thread，因而当系统中 workqueue 的数量很多时，就需要消耗大量的 PID 资源，同时大量的 worker thread 线程也会严重影响系统性能

> issue 2

其次，old-style workqueue 很容易发生阻塞。对于 singlethread workqueue，所有 CPU 共享一个 worker thread，当 worker thread 处理其中一个 work 阻塞时，之后等待的所有 work 都会阻塞在那里而得不到处理；对于 multi-thread workqueue，每个 CPU 都有一个绑定的 per-CPU worker thread，同一个 CPU 上的 work 共享一个 worker thread，因而在执行过程中当其中一个 work 阻塞时，该 CPU 上的其他 work 也必须等待

> issue 3

以上描述的 old-style workqueue 容易发生阻塞的问题，还可能会导致死锁问题。例如 work A 依赖于 work B，当这两个 work 都调度到同一个 worker thread 上时，如果 work A 先于 work B 执行，那么 work A 就会因为依赖于 work B 而进入阻塞，此时整个 worker thread 都进入阻塞状态，因而也调度不了 worker B，从而造成死锁


#### CMWQ

Old-style workqueue 在创建 workqueue 的过程中同时创建对应的 worker thread，此时 workqueue 与对应的 worker thread 具有严格的对应关系即二元性，这也是以上问题 1 的原因

CMWQ 为了解决这一问题，将 workqueue 与对应的 worker thread 相分离，由 worker-pool 管理和调度 worker thread，此时 workqueue 与 worker-pool 是彼此分离的。CMWQ 在初始化时只是创建有限数量的 worker-pool，worker thread 的调度全部由 worker-pool 管理，之后创建的所有的 workqueue 都共用这些全局的 worker-pool。由于 worker-pool 的数量是有限的，而每个 thread pool 中 worker thread 的数量也是严格控制的，因而就可以解决问题 1 中 worker thread 数量过多的问题


此外问题 2 的本质是由于 old-style workqueue 中虽然有 thread pool 的概念，但是这个 thread pool 并不是弹性的，即 thread pool 中 worker thread 的数量都是定死的，例如 multi-thread workqueue 对应的 thread pool 中 worker thread 的数量就是 CPU 的数量，singlethread workqueue 对应的 thread pool 中就只有一个 worker thread，当其中的任一个 worker thread 阻塞时，thread pool 中都不会再动态创建一个新的 worker thread 补上

问题 3 实际是问题 2 导致的，只要解决了问题 2，问题 3 自然也就不复存在

CMWQ 中 workqueue 与 worker-pool 是彼此分离的，worker thread 的调度全部由 worker-pool 管理，因而为了解决问题 2，CMWQ 中 worker-pool 是弹性可伸缩的，即当 worker thread 陷入阻塞时，worker-pool 会立即创建一个新的 worker thread 补上，以运行之后的 work


#### CMWQ concept

首先简单介绍 CMWQ workqueue 中的几个概念

work 抽象一个异步执行的任务，其包含的 func 字段描述异步执行的任务，将 work 添加到某个 workqueue 中以完成 work 的调度

workqueue 抽象任务 (work) 的集合

worker thread 抽象异步执行任务的线程（工人），worker thread 串行执行某个 workqueue 上的 work，同时由 thread-pool 对这些 worker thread 进行管理

thread pool 抽象工人（worker thread）的集合，负责调度并管理 thread-pool 中的 worker thread 

per-pool workqueue 抽象中介，负责建立 workqueue 与 worker pool 的关系
...
unbound worker-pool 是指不与单个 CPU 相绑定的 worker-pool，其中的 worker thread 可以在多个 CPU 上运行，这些 worker thread 具体可以在哪些 CPU 上运行，是由 unbound worker-pool 的 workqueue_attrs 属性来描述的
...
@nice 描述该 worker-pool 的 worker thread 的 nice 值

@cpumask 描述该 worker-pool 中的 worker thread 只能在该 cpumask 描述的 CPU 子集上运行，这是通过设置 worker thread 的 CPU affinity 实现的，至于 worker thread 最终是在 cpumask 中的哪一个 CPU 上运行的，则是由 scheduler 决定的
...
bound worker-pool 是 **per-CPU** 的 worker-pool，即每个 CPU 上都有一个对应的 worker-pool，这些 worker-pool 中的 worker thread 只能在当前对应的 CPU 上运行，
...
每个 CPU 实际上维护两个 bound thread pool，即 normal worker-pool 与 high priority worker-pool，两者的区别是，前者管理的 worker thread 的 nice 为默认的 0，而后者管理的 worker thread 的 nice 为 HIGHPRI_NICE_LEVEL 即 -20，因而后者管理的 worker thread 在调度时具有更高的优先级

bound worker-pool 同样由 workqueue_attrs 来描述其属性，例如对于 CPU 0 上的 high priority worker-pool，就具有如下 workqueue_attrs

- @cpumask 的值为 0x1，描述该 worker pool 中的 worker thread 只能在 CPU 0 上运行
- @nice 的值就为 -20
...
- <worker-ID> 表示该 worker thread 的编号
- 对于 high priority bound worker-pool，最后还会有 "H" 后缀


unbound worker thread 的命名规则为

```
"kworker/u<pool-ID>:<worker-ID>"
```

- unbound worker thread 都有一个前缀 "u"
...
bound workqueue 会与每个 CPU 上的 worker-pool 相联系，这样 workqueue 与 worker-pool 就是一对多的关系，此时需要 per-CPU 的 pool workqueue 结构来实现两者之间的联系
...
- 否则 @work->data 就保存了上一次处理该 work 的 bound worker-pool，此时
    - 如果 @work->data 指向的 worker-pool 当前存在 worker thread 正在运行这个 work，那么就会让当前这个 work 由 @work->data 指向的 worker-pool 运行，即在 @work->data 参数指向的 CPU 上运行
    - 否则还是选择 @cpu 参数对应的 bound worker-pool，即在 @cpu 参数描述的 CPU 上运行

所谓的 work 添加到 worker-pool 的过程，实际上就是将该 work 提交到对应的 worker-pool 的 @worklist 链表中去，并唤醒该 worker-pool 的 worker thread，之后 worker thread 被唤醒后就会处理 worklist 链表中的 work
...
此时只考虑系统中只包含一个 NUMA node 的情况，那么该 workqueue 只需要与一个 worker-pool 相联系，按理说此时不需要 pool workqueue 结构，因为只有当一个 workqueue 对应于多个 worker-pool，即 workqueue 与 worker-pool 是一对多的关系时，才需要 pool workqueue 结构来实现两者之间的联系；这里在 single NUMA node 的情况下，一个 workqueue 只对应于一个 worker-pool，但还是用了 pool workqueue 结构，这里是为了与之后 multi NUMA node 的情况保持兼容
...
考虑系统中存在多个 NUMA node 的情况，如果还是和之前一样，一个 workqueue 对应一个 worker pool，那么这个 worker pool 对应的 cpumask 参数就应该是 all_possible_cpu，即该 worker pool 下的 worker thread 可以在所有 NUMA node 的所有 CPU 下运行，这样就会带来一个问题，即 worker thread 可能被调度器调度到 NUMA node 2 的某个 CPU 上运行，而当初是 NUMA node 1 的某个 CPU 调用的 queue_work_on() 提交了这个 work，即这个 @work 数据大概率是存储在 NUMA node 1 上的，此时这个 worker thread 运行时就需要跨 NUMA node 访问 @work 数据，从而带来性能的下降

解决办法就是让 worker pool 是 per-NUMA node 的，即让一个 unbound workqueue 对应多个 worker pool，每个 NUMA node 一个 worker pool，其中的每个 worker pool 的 cpumask 参数只覆盖一个 NUMA node，即每个 per-NUMA-node worker-pool 的 worker thread 只能在该 NUMA node 对应的 CPU 上运行，之后 queue_work_on() 提交 work 的时候，实际上是将该 work 提交给当前 CPU 所在的 NUMA node 对应的那个 worker-pool 处理，这样 @work 数据是存储在当前 CPU 所在的 NUMA node 的，而实际执行的 worker thread 也是在同一个 NUMA node 上运行的，这样就不会再存在跨 NUMA node 的情况

由于此时一个 unbound workqueue 对应于多个 worker pool，即 workqueue 与 worker-pool 是一对多的关系，因而此时就需要 pool workqueue 结构来实现两者之间的联系
...
- 为 node 0 创建一个 unbound worker-pool，其 cpumask 为 CPU A/B，即其中的 worker thread 只能在 CPU A、CPU B 上运行
- 为 node 1 创建一个 unbound worker-pool，其 cpumask 为 CPU C/D，即其中的 worker thread 只能在 CPU C、CPU D 上运行
- 为 node 2 创建一个 unbound worker-pool，其 cpumask 为 CPU E/F，即其中的 worker thread 只能在 CPU E、CPU F 上运行
- 为 node 3 创建一个 unbound worker-pool，其 cpumask 为 CPU G/H，即其中的 worker thread 只能在 CPU G、CPU H 上运行
...
- 首先创建一个 default worker-pool，其 cpumask 参数即为该 standard unbound workqueue 的 workqueue attr 的 cpumask，即 @wq_unbound_cpumask 参数的值，即 CPU A、CPU C，也就是说 default worker-pool 的 worker thread 可以在 CPU A、CPU C 上运行
- 为 node 0 创建一个 unbound worker-pool，其 cpumask 为 CPU A，即其中的 worker thread 只能在 CPU A 上运行，之后在 node 0 即 CPU A、CPU B 上调用 queue_work_on() 时，work 都会提交到 node 0 对应的 worker-pool 
- 同理，为 node 1 创建一个 unbound worker-pool，其 cpumask 为 CPU C，即其中的 worker thread 只能在 CPU C 上运行，之后在 node 1 即 CPU C、CPU D 上调用 queue_work_on() 时，work 都会提交到 node 0 对应的 worker-pool
