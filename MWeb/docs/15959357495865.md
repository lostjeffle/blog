...
例如 virtio-blk 在下发一个 request 的过程中，会在 vring Descriptor Table 的 index 处设置一个对应的描述符，同时会在 descriptor state table 的 index 处设置一个 descriptor state，其 @data 字段存放 driver specific 的数据，这里对于 virtio-blk 来说，该字段就指向 struct virtblk_req 结构
