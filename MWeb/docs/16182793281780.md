...
实际上 readahead window size 是有可能超过 bdi->ra_pages 的，例如执行 POSIX_FADV_SEQUENTIAL fadvise() 的时候，readahead window size 实际上就是 2 倍的 bdi->ra_pages
...
另外当通过 force_page_cache_ra() 进入 readahead 路径时，实际上的 readahead window size (即 rac->_nr_pages) 是有可能大于 ra.ra_pages 的
...
> If the request exceeds the readahead window, allow the read to be up to the optimal hardware IO size

此时当 bdi->io_pages 大于 bdi->ra_pages 时，实际上的 readahead window size (即 rac->_nr_pages) 就会超过 ra.ra_pages
