...
虽然在 RCU lock 的保护下，xas_load() 的调用者在对返回的 xas->xa_node 进行引用的时候，并不会触发相应的操作，同时 xas->xa_node 的 @slots[xas->xa_offset] 位置处存储的值仍然有效 (即 entry A)，但是此时 xarray 的结构毕竟已经发生了变化，返回的 xas 作为指向 xarray 的一个指针，其已然失效，例如对 xas_load() 返回的 xas 执行 xas_advance() 就会导致错误
