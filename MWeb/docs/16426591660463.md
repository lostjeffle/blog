...
之前介绍的 RCU locking 可以让 xarray 的 read/write 类操作并行执行的同时，方式 read 类操作引用到已经释放了的内存
...
虽然在 RCU lock 的保护下，xas_load() 的调用者在对返回的 xas->xa_node 进行引用的时候，并不会触发相应的操作，同时 xas->xa_node 的 @slots[xas->xa_offset] 位置处存储的值仍然有效 (即 entry A)，但是此时 xarray 的结构毕竟已经发生了变化，返回的 xas 作为指向 xarray 的一个指针，其已然失效
...
这样 xas_load() 的调用者必须对 xas_load() 返回的结果进行检查，如果最终返回的 entry 实际上是 XA_RETRY_ENTRY，就说明之前一轮 xas_load() 执行过程中并发执行的 write 操作，已经导致 xarray 的结构发生了变化；此时程序必须重新发起一轮新的 xas_load() 查找操作，从 xarray 树的顶点开始重新查找
