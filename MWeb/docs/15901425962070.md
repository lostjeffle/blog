...
page cache 的 cleanup 操作分为 reclaim 和 invalidate 两种

#### reclaim

reclaim 操作是指系统中内存不足触发内存回收，或者通过 /proc/sys/vm/drop_caches 触发 page cache 回收时，需要对 page cache 执行 reclaim 操作，即将对应的 page cache (folio) 从 address space 中移除，之后回收这些 page cache (folio) 的内存

reclaim 过程中会调用到文件系统的 a_ops->release_folio() 回调函数；reclaim 操作的目的是尽可能地回收一些内存，因而过程中只会回收 clean page，对于 dirty/locked/mmaped page 都是跳过的

##### invalidate_mapping_pages

reclaim 操作的入口为 invalidate_mapping_pages()，例如通过 /proc/sys/vm/drop_caches 触发 page cache 回收时，就会调用到这个函数
...
invalidate_mapping_pages() 对 address space 中 [start, end) 区间内的 page cache 执行 reclaim 操作

如之前所述，reclaim 操作的目的是尽可能地回收内存，因而 invalidate_mapping_pages() 中只会回收 clean unused page，以下 page 不会被移除
...
invalidate 操作是指将 address space 中特定范围内的 page cache 作 invalidate 操作，即将这个范围内的 page cache 从 address space 中移除，同时释放这些 page cache (folio) 占用的内存

与之前描述的 reclaim 操作的差异是，reclaim 操作的目的是回收一些内存，因而对于一些特定的 page 是可以失败的，例如对于 dirty/locked/mmaped page 就是跳过的；而 invalidate 操作通常是在对文件执行 truncate 操作使文件缩小的时候触发的，因而必须将 newsize 之后的 page cache 释放掉，不允许失败；或者是文件执行 evict 操作的时候，必须将 address space 中的所有 page cache 都释放掉，也不允许失败


invalidate 过程中会调用到文件系统的 a_ops->invalidate_folio() 回调函数

之前介绍过，reclaim 过程中会调用到文件系统的 a_ops->release_folio() 回调函数，这两个回调函数其实都是用于释放 folio->private 中保存的 private data，例如 fiemap 中的 buffer_head、或者 iomap 中的 iomap_page；但两者的差别是，reclaim 操作对于特定的 folio 是允许失败的，因而 release_folio() 回调函数也是可以允许失败的，例如对于 dirty/writeback page 并不会释放 folio->private，同时回调函数返回 false；而 invalidate 操作是不允许失败的，因而 invalidate_folio() 回调函数中无论如何都需要释放 folio->private，同时这个回调函数的返回值类型也是 void


##### truncate_inode_pages

invalidate 操作的入口为 truncate_inode_pages()，例如 evict() 过程中就会调用到这个函数
