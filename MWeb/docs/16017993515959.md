title:'IO - iomap - 4 Direct IO'
## IO - iomap - 4 Direct IO
...
之后当 iomap_dio->ref 计数降为 0 即所有对应的 bio 都完成时，就会调用 kiocb->ki_complete() 回调函数
...
当 DIRECT IO 执行完成的时候，iomap_dio_rw() 会返回实际成功读取/写入的数据量
...
对于 sync DIRECT IO 来说，iomap_dio_rw() 中会同步等待 IO 执行完成，因而 iomap_dio_rw() 的返回值就描述了成功访问的数据量；发生错误的时候会返回负的错误码

而对于 async DIRECT IO 来说，iomap_dio_rw() 中下发 IO 之后就会立即返回 -EIOCBQUEUED，(当发生其他错误的时候会返回其他负的错误码)；而之后当 IO 真正执行完成的时候就会调用 @iocb->ki_complete() 回调函数
...
iomap_dio_rw() 是有可能返回 0 的，例如传入的 @iter->count 为 0 的时候就会返回 0；当 READ IO 尝试读取超过文件大小的数据 (@iocb->ki_pos 超过 @i_size) 时也会返回 0
