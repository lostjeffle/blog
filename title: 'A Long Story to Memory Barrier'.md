title: 'A Long Story to Memory Barrier'
status: 'draft'

# A Long Story to Memory Barrier
...
但是由于这种优化是在编译器 (compiler reordering) 与 CPU (CPU reordering) 层面进行的，编译器与 CPU 并不知道上层软件的设计是 single-threaded 的还是 multi-threaded 的

如果是 single-threaded 的，那么 memory reordering 这种优化在提高代码运行速度的同时，并不会对程序的语义带来任何问题；然而如果是 multi-threaded 的，那么这种优化有时就会对程序的语义带来问题
...
**introduction of compiler reordering**

编译器在编译过程中可能会采取一系列的编译优化，例如使用寄存器 (register) 缓存内存中变量的值、指令重排等，这些优化的前提是在 single-threaded program 中这些优化并不能影响程序功能 (functionality) 的正常实现

但是在 multi-threaded 程序中，这些优化有时就会带来问题，考虑以下程序
...
这正是因为编译器使用寄存器对内存的值进行缓存的编译优化，在 single-threaded 程序中这种优化自然无伤大雅，然而在 multi-threaded 程序中，其中一个线程会陷入无限循环，即使其他线程已经修改了全局变量 a 的值


**compiler barrier**

导致这种问题的根本原因是，编译器层面的优化不可能知道上层软件的具体设计，编译器不知道上层软件到底设计为 single-threaded 还是 multi-threaded，因而解决办法是由上层软件的设计者显式地告诉编译器，在读取特定变量的时候，从内存读取该变量的值，而不是复用之前读取的缓存在寄存器中的值

barrier() 也就是 compiler barrier 就用于实现这一目的，其具体实现是编译器相关的，gcc 版本的 compiler barrier 在 `include/linux/compiler-gcc.h` 中定义

```c
#define barrier()__asm__ __volatile__("": : :"memory")
```

gcc 中的 compiler barrier 实际上是一个空指令，其中只是用到了一个 "memory" clobber，其解释为
...
因而 barrier()实际上是告诉编译器，这条指令可能会读取或写入任何内存地址，那么编译器会变得保守起来，使得该条指令之前的内存访问操作不会移到这条指令后面，该条指令之后的内存访问操作也不会移到这条指令前面，从而防止编译时编译器优化造成的指令顺序重排，从而确保 barrier() 前后的代码块的相对顺序

此外 barrier() 还有一个副作用是，编译器会将所有寄存器中缓存的值下刷到内存，之后再重新读取内存中的值并缓存到寄存器中

compiler barrier 并不对应任何一条指令，它只是告诉编译器生成不同风格（也就是去除编译优化）的指令，因而 compiler barrier 本身基本没有什么开销
...
**effect of compiler barrier**

接下来我们看一下 compiler barrier 的效果
...
可以看到加上 compiler barrier 之后，恢复为从内存读取变量的值


### why volatile deprecated

上述例子中 compiler barrier 主要是抑制编译器的编译优化，很自然地我们会想到 C 语言中的 volatile 关键词
...
虽然 compiler barrier 和 volatile 都有抑制编译器优化的效果，但是两者还是存在着细微的差别
...
同样的例子，再来看一下使用 compiler barrier 的效果
...
可以看到每次循环执行一次读内存的操作，同时在执行 b=a 时服用寄存器中缓存的 a 值


从以上的例子中可以看出，相对于 volatile，compiler barrier 显然是一个更好的选择

实际上 volatile 关键词一开始就是用于修饰 IO mapped memory 中的变量，这些变量实际上映射为硬件处理器的寄存器空间，这些变量的值随时有可能被硬件处理器修改，因而 CPU 在访问这些变量的时候，不能复用之前寄存器中缓存的值，而必须重复读取内存中的值
...
memory reordering 的另一个来源则是 CPU reordering，发生在 CPU 运行阶段，下面从两个例子介绍 CPU reordring 发生的原因

*本节参考 “Is Parallel Programming Hard, And, If So, What Can You Do About It?”*

#### store buffer

memory reordering 的第一个例子与 CPU 使用的 store buffer 相关

**introduction of store buffer**
...
**memory reordering caused by store buffer**

store buffer 可以提高一个 CPU 指令执行的速度，在 SMP 系统中当多个线程或进程在多个 CPU 上并行运行时，store buffer 特性就会造成 memory reordering
...
**introduction of invalidate queue**
...
**memory reordering caused by invalidate queue**

然而 invalidate queue 机制的引入又会带来新的 memory reordering 问题
...
因而在 CPU 1 的两次 load 操作之间也调用 smp_mb()memory barrier，smp_mb() memory barrier 会对 invalidate queue 中的 pending invalidate message 对应的 cache line 进行标记，smp_mb() memory barrier 之后的 load 操作对应的 cache line 被标记时，该 load 操作就会一直等待对应的 pending invalidate message 处理完成

即 smp_mb() memory barrier 会等待 invalidate queue 中的所有 pending invalidate message 处理完成，再继续执行 memory barrier 之后的 load 操作
...
以上介绍的 compiler barrier 和 CPU barrier 实际上都属于 memory barrier

我们日常编程中可能比较少关注 memory barrier，这是因为我们日常使用的 spinlock/mutex 等锁机制内部已经使用到 memory barrier，因而只有无锁编程即访问共享变量，同时又不使用锁机制进行保护的场景下，才需要显式使用 memory barrier


### ACCESS_ONCE()

[reflink: ACCESS_ONCE()](https://lwn.net/Articles/508991/)

内核中的 ACCESS_ONCE() 实际上用作 compiler barrier
...
激进的编译器可能认为 lock->owner 的值在当前线程中永远不会改变，因而优化为
...
但是我们知道 lock->owner 有可能被其他线程改变，因而这里必须用 compiler barrier 抑制编译器的优化


内核中使用 ACCESS_ONCE() 作为 compiler barrier，其定义为

```c
#define ACCESS_ONCE(x) (*(volatile typeof(x) *)&(x))
```

这里是使用 volatile 来抑制编译器的优化，需要注意的是

- 与 volatile 的区别：和之前介绍的将整个变量声明为 volatile 不同，这里通过指针的方式，将变量**临时地**转化为 volatile 变量，相当于只有调用 ACCESS_ONCE() 的时候才会抑制编译器优化，而在其他地方访问变量的时候，编译器优化是正常开启的
- 与 barrier() 的区别：barrier() 会下刷所有寄存器，相当于是对所有变量都抑制编译器优化，而通过 volatile 可以实现对特定变量定向地抑制编译器优化


### READ_ONCE()

[reflink: ACCESS_ONCE() and compiler bugs](https://lwn.net/Articles/624126/)

READ_ONCE() 是 ACCESS_ONCE() 的升级版本，由于 ACCESS_ONCE() 自身存在的缺陷，内核代码逐渐使用 READ_ONCE() 替代 ACCESS_ONCE()，最终在 v4.15 移除 ACCESS_ONCE()
...
## When should we use memory barrier?

> compiler barrier

理论上当一个变量在两个或多个线程之间共享时，同时其中一个线程会对变量执行修改 (write) 操作，那么另一个线程在读取 (read) 这个变量的时候，就需要使用 READ_ONCE() 读取该变量
